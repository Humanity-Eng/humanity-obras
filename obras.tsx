...
const VERSAO="2026.08.19 · build 23";
 ...
 for(const k of Object.keys(defs)){
     ...
     if(k==="acomp"&&obraId==="lote_d18"&&v&&Array.isArray(v)&&v.length>0){
      ...
      if(k==="compras"&&chaveDef==="madalena"&&Array.isArray(v)){
       // As compras da Jurunense são acrescentadas uma única vez, preservando
       // tudo que já foi cadastrado na aba.
       const temJur=v.some(c=>String(c.obs||"").toUpperCase().indexOf("JURUNENSE")>=0);
       if(!temJur){const merged=v.concat(CP_MADALENA);nd[k]=merged;dbSet(obraId,k,merged);}
       else nd[k]=v;
      }
