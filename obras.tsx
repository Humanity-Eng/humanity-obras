...
export default function App(){
 ...
 useEffect(()=>{
  (async()=>{
   ...
   if(lista&&Array.isArray(lista)&&lista.length>0)setObras(lista);
   else dbSet("_sistema","obras",OBRAS_SEED);
  })();
 },[]);
 useEffect(()=>{
  if(!D||!D.msgs)return;
 ...
 {tab===6&&<Cronograma D={D} obra={obra} sv={v=>sv("crono",v)}/>}
...
// Orçamentos por obra
const ORC_POR_OBRA={lote_d18:ORC,crtr14:ORC_CRTR14};
...
// As etapas ocupam janelas sequenciais com sobreposição configurável (o padrão
// de mercado: a etapa seguinte começa antes da anterior terminar), gerando a
...
// mão de obra e demais indiretos acompanham o avanço físico.
...
function projetarEmprestimos(EM,baseData,meses){
 ...
 const detalhes=[];
 let amortizacao=0,economia=0,totalNoMes=0;
 (EM||[]).forEach(e=>{
  ...
  if(!baseData||!n||!parcela)return;
  const d1=parseBRDate(e.dataParcela1)||(parseBRDate(e.data)?new Date(parseBRDate(e.data).getFullYear(),parseBRDate(e.data).getMonth()+1,parseBRDate(e.data).getDate()):null);
  if(!d1)return;
  ...
  // Taxa usada na quitação = a do contrato (informada ou implícita sobre o valor contratado).
  // O IOF e as tarifas já foram pagos na liberação e não voltam na antecipação.
  const informada=Number(e.taxaMes)>0?Number(e.taxaMes)/100:0;
  const i=informada||taxaMensal(contratado||liberado,parcela,n);
  const cet=taxaMensal(liberado||contratado,parcela,n); // custo efetivo, já com IOF/tarifas
  ...
  totalNoMes+=dentro*parcela+quit;
 });
 return{porMes,detalhes,amortizacao,economia,total:totalNoMes};
}

function Cronograma({D,obra,sv}){
 const A=D.acomp||[];const C=D.custos||[];
 const cfg=D.crono||{};
 ...
 const mesesPadrao=Math.max(2,Math.round(diasTot/30/2)); // dias de atividade com paralelismo médio de 2 frentes
 ...
 // Custos que não têm etapa física correspondente (administrativo, impostos, mão de obra…)
 const indiretos=Object.keys(custoLancado)
  .filter(p=>nomesEtapas.indexOf(p)<0)
  .map(p=>{
   const tipo=(cfg.tipos&&cfg.tipos[p])||(ehLinear(p)?"linear":"fisico");
   return{nome:p,custo:custoDe(p),tipo};
  }).filter(x=>x.custo>0);
 ...
 const salvarCustoEtapa=()=>{
  sv({...cfg,custos:{...(cfg.custos||{}),[editCusto]:Number(fCusto)||0}});
  setEditCusto(null);setFCusto("");
 };
 const limparCustoEtapa=nome=>{
  const nc={...(cfg.custos||{})};delete nc[nome];sv({...cfg,custos:nc});setEditCusto(null);
 };

 // ── Distribuição mensal ──
 const EMP=D.emprestimos||[];
 const projEmp=projetarEmprestimos(EMP,baseData,meses);
 ...
 for(let m=0;m<meses;m++){
  ...
  linhas.push({m,fis,dir,ativas});
  acFis+=fis;
 }
 const somaFis=linhas.reduce((s,l)=>s+l.fis,0)||1;
 linhas.forEach(l=>{
  const propor=totalIndireto>0?indiretos.filter(x=>x.tipo==="fisico").reduce((s,x)=>s+x.custo,0)*(l.fis/somaFis):0;
  const linear=indiretos.filter(x=>x.tipo==="linear").reduce((s,x)=>s+x.custo,0)/meses;
  l.indireto=propor+linear;
  l.emp=projEmp.porMes[l.m]||0;
  l.fin=l.dir+l.indireto+l.emp;
 });
 const desembolsoTotal=custoTotal+totalEmp;
 acFis=0;acFin=0;
 linhas.forEach(l=>{acFis+=l.fis;acFin+=l.fin;l.acFis=acFis;l.acFin=acFin;});

 // ── Realizado ──
 const realFis=calcConclusaoGeral(A);
 const pagoTot=C.reduce((s,c)=>s+(Number(c.pago)||0),0);
 const hoje=new Date();
 const mesesDecorridos=baseData?((hoje.getFullYear()-baseData.getFullYear())*12+(hoje.getMonth()-baseData.getMonth()))+(hoje.getDate()/30):0;
 const mesAtual=baseData?Math.max(1,Math.min(Math.ceil(mesesDecorridos),meses)):0;
 const tHoje=baseData?Math.max(0,Math.min(mesesDecorridos/meses,1)):0;
 // previsto acumulado até hoje (interpolado dentro do mês)
 const acumAte=(t,campo)=>{
  const mIdx=Math.floor(t*meses);
  let ac=0;
 ...
 const prevFisHoje=baseData?acumAte(tHoje,"fis"):0;
 const prevFinHoje=baseData?acumAte(tHoje,"fin"):0;
 ...
 const desembRest=Math.max(desembolsoTotal-pagoTot,0)/mesesRest;

 // Realizado físico por mês (a partir das datas das medições)
 const realPorMes={};
 A.forEach(a=>{
  (a.meds||[]).forEach(md=>{
   const d=parseBRDate(md.data);
   if(!d||!baseData||!a.total)return;
   const idx=(d.getFullYear()-baseData.getFullYear())*12+(d.getMonth()-baseData.getMonth());
   if(idx<0||idx>=meses)return;
   realPorMes[idx]=(realPorMes[idx]||0)+((Number(md.qtd)||0)/a.total)*(Number(a.prazo)||0)/diasTot*100;
  });
 });
 const temReal=Object.keys(realPorMes).length>0;
 let acR=0;const realAcum=[];
 for(let m=0;m<meses;m++){acR+=(realPorMes[m]||0);realAcum.push(acR);}
 // diferença entre o avanço real total e o que está datado nas medições (histórico sem data)
 const semData=Math.max(realFis-acR,0);
 ...
 const pathFin=linhas.map((l,i)=>(i?"L":"M")+px(i).toFixed(1)+" "+py(desembolsoTotal>0?l.acFin/desembolsoTotal*100:0).toFixed(1)).join(" ");
 const ptsReal=[];
 if(temReal){
  const lim=Math.min(mesAtual,meses);
  for(let m=0;m<lim;m++)ptsReal.push({x:px(m),y:py(realAcum[m]+semData)});
 }
 const pathReal=ptsReal.map((p,i)=>(i?"L":"M")+p.x.toFixed(1)+" "+p.y.toFixed(1)).join(" ");
  ...
  <div>
   ...
   {desembolsoTotal>0&&(
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:12,marginBottom:16}}>
     ...
     <KCard label="Desembolso Total" value={fmtBR(desembolsoTotal)} color="#f59e0b" sub={"em "+meses+" meses · média "+fmtBR(desembolsoTotal/meses)+"/mês"}/>
    </div>
   )}
   ...
   <div style={{background:"#1e2d5a",borderRadius:12,padding:16,marginBottom:16}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:editando?12:0}}>
     <div style={{color:"#fff",fontSize:13,fontWeight:700}}>
      {meses} meses · início {inicioStr||"não informado"} · sobreposição {(sobrep*100).toFixed(0)}%
      <div style={{fontSize:11,color:"#94a3b8",fontWeight:400,marginTop:3}}>Sobreposição = quanto a etapa seguinte começa antes da anterior acabar.</div>
     </div>
   ...
   {/* Situação hoje */}
   {baseData&&(
    <Bloco titulo={"Situação hoje — mês "+mesAtual+" de "+meses} cor={desvioFis>=0?"#16a34a":"#ef4444"}>
     <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:10,marginBottom:12}}>
      ...
      <div><div style={{fontSize:10,color:"#64748b",fontWeight:700,textTransform:"uppercase"}}>Desembolso previsto</div><div style={{fontSize:17,fontWeight:800,color:"#1e293b"}}>{fmtBR(prevFinHoje)}</div></div>
      <div><div style={{fontSize:10,color:"#64748b",fontWeight:700,textTransform:"uppercase"}}>Já pago</div><div style={{fontSize:17,fontWeight:800,color:"#8b5cf6"}}>{fmtBR(pagoTot)}</div></div>
     </div>
   ...
   {/* Curva S */}
   <Bloco titulo="Curva S — previsto x realizado" cor="#3b82f6">
    <svg viewBox={"0 0 "+W+" "+H} style={{width:"100%",height:"auto"}}>
     ...
     {pathReal&&<path d={pathReal} fill="none" stroke="#16a34a" strokeWidth="2.4"/>}
     {ptsReal.map((p,i)=><circle key={i} cx={p.x} cy={p.y} r="2.6" fill="#16a34a"/>)}
    </svg>
    <div style={{display:"flex",gap:14,flexWrap:"wrap",fontSize:11,color:"#64748b",marginTop:8}}>
     <span><b style={{color:"#f59e0b"}}>—</b> físico previsto</span>
     <span><b style={{color:"#ef4444"}}>- -</b> financeiro previsto</span>
     {temReal&&<span><b style={{color:"#16a34a"}}>—</b> físico realizado</span>}
    </div>
    {semData>0.01&&<div style={{fontSize:10,color:"#94a3b8",marginTop:6}}>{pct(semData)} do avanço vem do histórico anterior às medições datadas e está somado ao ponto inicial da curva realizada.</div>}
   </Bloco>

   {/* Tabela mensal */}
   <Bloco titulo="Previsão mês a mês" cor="#f8c400">
    <div style={{fontSize:11,color:"#64748b",marginBottom:10}}>Toque em um mês para ver o que se executa e quanto custa.</div>
    <div style={{overflowX:"auto"}}>
     <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
      <thead><tr style={{background:"#f8fafc"}}>
       {["Mês","Físico","Acum.","Desembolso","Acum."].map(h=><th key={h} style={{padding:"8px 10px",textAlign:"left",color:"#64748b",fontWeight:700,whiteSpace:"nowrap"}}>{h}</th>)}
      </tr></thead>
      <tbody>
       {linhas.map((l,i)=>{
        const atual=baseData&&(i+1)===mesAtual;
        return(
         <tr key={i} onClick={()=>setDetalhe(detalhe===i?null:i)} style={{borderBottom:"1px solid #f1f5f9",cursor:"pointer",background:atual?"#fffbeb":detalhe===i?"#f8fafc":"transparent"}}>
          <td style={{padding:"9px 10px",fontWeight:700,color:atual?"#92400e":"#1e293b",whiteSpace:"nowrap"}}>{detalhe===i?"▾ ":"▸ "}{mesLabel(baseData,i)}{atual?" •":""}</td>
          <td style={{padding:"9px 10px",color:"#f59e0b",fontWeight:700}}>{l.fis.toFixed(1)}%</td>
          <td style={{padding:"9px 10px",color:"#64748b"}}>{l.acFis.toFixed(1)}%</td>
          <td style={{padding:"9px 10px",color:"#ef4444",fontWeight:700,whiteSpace:"nowrap"}}>{fmtBR(l.fin)}</td>
       ...
       <tr style={{background:"#1e2d5a",color:"#fff"}}>
        ...
        <td/>
        <td style={{padding:"9px 10px",fontWeight:800,color:"#f8c400",whiteSpace:"nowrap"}}>{fmtBR(desembolsoTotal)}</td>
        <td/>
       </tr>
      </tbody>
     </table>
   ...
   {detalhe!==null&&linhas[detalhe]&&(
    <Bloco titulo={"O que se executa em "+mesLabel(baseData,detalhe)} cor="#0ea5e9">
     <div style={{fontSize:12,color:"#475569",marginBottom:12,lineHeight:1.6}}>
      Avanço previsto de <b style={{color:"#f59e0b"}}>{linhas[detalhe].fis.toFixed(2)}%</b> da obra e desembolso de <b style={{color:"#ef4444"}}>{fmtBR(linhas[detalhe].fin)}</b> — sendo {fmtBR(linhas[detalhe].dir)} em serviços, {fmtBR(linhas[detalhe].indireto)} de custos diluídos (administrativo, impostos, mão de obra){linhas[detalhe].emp>0?" e "+fmtBR(linhas[detalhe].emp)+" de empréstimo":""}.
     ...
     {linhas[detalhe].ativas.map((at,ai)=>{
      // subdivisões da etapa que caem neste mês (janelas sequenciais dentro da etapa)
      const secs=[];
   ...
   {projEmp.detalhes.length>0&&(
    <Bloco titulo="Empréstimos dentro do cronograma" cor="#0ea5e9">
     <div style={{fontSize:11,color:"#64748b",marginBottom:10,lineHeight:1.5}}>As parcelas entram no desembolso do mês do vencimento. As que venceriam depois da entrega da obra são projetadas como <b>quitação antecipada no último mês</b>, trazidas a valor presente pela taxa do contrato — IOF e tarifas não entram nesse desconto porque já foram pagos na liberação.</div>
      ...
      <div key={i} style={{background:"#f8fafc",borderRadius:10,padding:"10px 12px",marginBottom:8}}>
       ...
       {x.fora>0&&(
        <div style={{background:"#e0f2fe",border:"1px solid #bae6fd",borderRadius:8,padding:"8px 10px",marginTop:8,fontSize:11,color:"#075985",lineHeight:1.6}}>
         💡 <b>{x.fora} parcela(s)</b> venceriam depois da entrega ({fmtBR(x.face)} em valor de face). Quitando de uma vez em {mesLabel(baseData,meses-1)} com o recebimento final: <b>{fmtBR(x.quit)}</b> — economia de <b>{fmtBR(x.economia)}</b> em juros futuros, descontados a {(x.taxa*100).toFixed(2)}% a.m.{x.informada?"":" Informe a taxa do contrato na aba Empréstimo para o valor ficar exato."}
        </div>
       )}
      </div>
     ))}
     <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:10,marginTop:10,paddingTop:10,borderTop:"1px solid #f1f5f9"}}>
      <div><div style={{fontSize:10,color:"#64748b",fontWeight:700,textTransform:"uppercase"}}>Empréstimo no período</div><div style={{fontSize:15,fontWeight:800,color:"#0ea5e9"}}>{fmtBR(totalEmp)}</div></div>
      <div><div style={{fontSize:10,color:"#64748b",fontWeight:700,textTransform:"uppercase"}}>Quitação no último mês</div><div style={{fontSize:15,fontWeight:800,color:"#1e293b"}}>{fmtBR(projEmp.amortizacao)}</div></div>
      <div><div style={{fontSize:10,color:"#64748b",fontWeight:700,textTransform:"uppercase"}}>Economia projetada</div><div style={{fontSize:15,fontWeight:800,color:"#16a34a"}}>{fmtBR(projEmp.economia)}</div></div>
     </div>
    </Bloco>
   )}
   ...
   <Bloco titulo="Etapas — peso físico, janela e custo" cor="#8b5cf6">
    ...
    {indiretos.length>0&&(
     <div style={{marginTop:14}}>
      ...
      <div style={{fontSize:10,color:"#94a3b8",marginTop:6,lineHeight:1.5}}>Administrativo, impostos e taxas entram divididos igualmente por mês; mão de obra e demais indiretos acompanham o ritmo da obra. Toque para trocar o critério.</div>
     </div>
    )}
   </Bloco>
   ...
   <div style={{background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:10,padding:"12px 14px",fontSize:11,color:"#64748b",lineHeight:1.6}}>
    <b style={{color:"#1e2d5a"}}>Como este cronograma é montado:</b> o peso físico de cada etapa vem dos dias de atividade cadastrados (mesma base do percentual de conclusão). As etapas são posicionadas em sequência com sobreposição, o que produz a curva S — início lento, pico no meio da obra e desaceleração no acabamento. O custo de cada etapa é distribuído ao longo da própria janela; administrativo, impostos e taxas são diluídos igualmente por mês. É uma previsão de cenário ideal: ajuste o prazo e os custos por etapa conforme a realidade da obra.
   </div>
  </div>
 );
}
