...
export default function App(){
 ...
 if(!obraId)return <ObrasList user={user} obras={obras} onSel={id=>{setObraId(id);setTab(0);}} onOut={()=>setUser(null)}
    onNova={o=>{
     ...
     const vazio={caixa:{saldo:0,hist:[]},custos:[],acomp:[],estoque:[],consumo:[],msgs:[],compras:[],rec:[],emp:[]};
 ...
 const TA=["Dashboard","Orçamento","Custos","Recebimentos","Caixa","Medições","Empreitada","Estoque","Compras","Comunicação"];
 ...
 {tab===5&&<Medicoes A={D.acomp||[]} sv={v=>sv("acomp",v)} obra={obra}/>}
 {tab===6&&<Empreitada E={D.emp||[]} sv={v=>sv("emp",v)} A={D.acomp||[]} svA={v=>sv("acomp",v)}/>}
 {tab===7&&<Estoque ES={D.estoque||[]} CON={D.consumo||[]} svE={v=>sv("estoque",v)} svC={v=>sv("consumo",v)}/>}
 {tab===8&&<Compras CP={D.compras||[]} sv={v=>sv("compras",v)} svE={v=>sv("estoque",v)} ES={D.estoque||[]}/>}
     {tab===9&&<Comunicacao C={D.custos||[]} sv={v=>sv("custos",v)} msgs={D.msgs||[]} svM={v=>sv("msgs",v)} user={user}/>}
 </>}
 {role==="eng"&&<>
...
function ObrasList({user,obras,onSel,onOut,onNova,onDel}){
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:16}}>
     {visiveis.map(o=>(
      <div key={o.id} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:14,padding:24,boxShadow:"0 1px 4px #0001",position:"relative"}}>
       ...
       {user.role==="admin"&&o.id!=="lote_d18"&&(
        <button onClick={()=>{if(confirm("Excluir a obra \""+o.nome+"\" e todos os seus dados?"))onDel(o.id);}} style={{position:"absolute",top:10,right:10,background:"#fee2e2",border:"none",color:"#ef4444",borderRadius:6,padding:"3px 8px",cursor:"pointer",fontSize:11,fontWeight:700}}>Excluir</button>
       )}
      </div>
     ))}
     {user.role==="admin"&&(
...
function Dashboard({D,obra}){
 ...
 const contratoV=obra?obra.contrato:0;
 const aReceberV=Math.max(contratoV-tR,0);
 const aPagarV=Math.max(tC-tP,0);
 const lucroRealV=(Number(caixaObj.saldo)||0)+aReceberV-aPagarV;
 ...
 return(
  <div>
   ...
   <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12,marginBottom:24}}>
    ...
    <KCard label="Lucro Real Projetado" value={fmtBR(lucroRealV)} color={lucroRealV>=0?"#059669":"#ef4444"} sub={"Caixa "+fmtBR(Number(caixaObj.saldo)||0)+" · ver aba Caixa"}/>
   </div>
...
function Caixa({D,obra,sv}){
 ...
 const aReceber=Math.max(contrato-jaRecebido,0);
 const aPagar=Math.max(custoTotal-jaPago,0);
 const saldo=Number(caixa.saldo)||0;
 const lucroReal=saldo+aReceber-aPagar;
 const lucroEstimado=contrato-custoTotal;
 ...
 return(
  <div>
   ...
   <div style={{background:"#1e2d5a",borderRadius:14,padding:20,marginBottom:20,textAlign:"center"}}>
    ...
    <div style={{fontSize:12,color:"#94a3b8",marginTop:8}}>Caixa {fmtBR(saldo)} + a receber {fmtBR(aReceber)} − a pagar {fmtBR(aPagar)}</div>
   </div>
   ...
   <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12,marginBottom:20}}>
    ...
    <Card label="A Receber (cliente)" value={aReceber} color="#3b82f6" sub={`de ${fmtBR(contrato)} · recebido ${fmtBR(jaRecebido)}`}/>
    <Card label="A Pagar (custos)" value={aPagar} color="#ef4444" sub={`de ${fmtBR(custoTotal)} · pago ${fmtBR(jaPago)}`}/>
    <Card label="Lucro Estimado (fim)" value={lucroEstimado} color="#10b981" sub="contrato − custo total"/>
   </div>
