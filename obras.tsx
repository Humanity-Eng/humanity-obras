...
const VERSAO="2026.08.15 · build 14";
...
export default function App(){
 ...
 ],rec:IR,emp:IE_DEFAULT,emprestimos:[],lidos:{},crono:{},tarefas:[]};
 ...
 const CHAVES_MADALENA=["acomp","custos","rec","crono","tarefas"];
 if(chaveDef==="madalena"){
  defs.acomp=IA_MADALENA;defs.custos=IC_MADALENA;defs.rec=IR_MADALENA;
  defs.crono={inicio:"01/08/2026",meses:6};defs.tarefas=IT_MADALENA;
 }
 const nd={};
 ...
 if(!obraId)return <ObrasList user={user} obras={obras} onSel={id=>{setObraId(id);setTab(0);}} onOut={()=>setUser(null)}
    onNova={o=>{
     ...
     const vazio={caixa:{saldo:0,hist:[]},custos:[],acomp:[],estoque:[],consumo:[],msgs:[],compras:[],rec:[],emp:[],emprestimos:[],lidos:{},crono:{},tarefas:[]};
 ...
 const TA=["Dashboard","Atividades","Orçamento","Custos","Recebimentos","Caixa","Empréstimo","Cronograma","Medições","Empreitada","Estoque","Compras","Comunicação"];
 ...
 {tab===11&&<Compras CP={D.compras||[]} sv={v=>sv("compras",v)} svE={v=>sv("estoque",v)} ES={D.estoque||[]}/>}
     {tab===12&&<Comunicacao C={D.custos||[]} sv={v=>sv("custos",v)} msgs={D.msgs||[]} svM={v=>sv("msgs",v)} user={user}/>}
 </>}
 {role==="eng"&&<>
