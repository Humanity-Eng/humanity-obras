...
export default function App(){
 ...
 for(const k of Object.keys(defs)){
     let v=await dbGet(obraId,k);
     if(v===null){
      // Tenta migrar do localStorage (dados salvos antes do Supabase)
      try{
       const lsKey=obraId+":"+k;
       const ls=localStorage.getItem(lsKey);
       if(ls){v=JSON.parse(ls);await dbSet(obraId,k,v);localStorage.removeItem(lsKey);}
      }catch(ex){}
     }
     nd[k]=v||defs[k];
    }
 ...
 const sv=(k,v)=>{
  setD(d=>({...d,[k]:v}));
  dbSet(obraId,k,v);
  try{localStorage.setItem(obraId+":"+k,JSON.stringify(v));}catch(ex){}
 };
