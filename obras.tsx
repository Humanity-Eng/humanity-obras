...
function fmtBRDate(d){return d?D2(d.getDate())+"/"+D2(d.getMonth()+1)+"/"+d.getFullYear():"";}
// Base global das semanas (definida pela obra aberta)
let SEM_BASE=null;
function setSemBase(inicio){SEM_BASE=parseBRDate(inicio);}
function semRange(n){
 if(!SEM_BASE)return null;
 const ini=new Date(SEM_BASE.getTime()+(n-1)*7*86400000);
 const fim=new Date(ini.getTime()+6*86400000);
 return{ini,fim};
}
function getSemanaAtual(){
 if(SEM_BASE){
  const diff=Math.floor((new Date().getTime()-SEM_BASE.getTime())/(7*86400000));
  return Math.max(1,diff+1);
 }
 const base=new Date(2026,5,1);
 return Math.max(1,Math.floor((new Date()-base)/(7*86400000))+1);
}
function semLabel(n){
 const r=semRange(n);
 if(r)return"S"+n+" ("+D2(r.ini.getDate())+"/"+D2(r.ini.getMonth()+1)+"–"+D2(r.fim.getDate())+"/"+D2(r.fim.getMonth()+1)+")";
 return"S"+n;
}
...
export default function App(){
 ...
 ],rec:IR,emp:IE_DEFAULT,emprestimos:[],lidos:{}};
 const nd={};
 ...
 if(!obraId)return <ObrasList user={user} obras={obras} onSel={id=>{setObraId(id);setTab(0);}} onOut={()=>setUser(null)}
    onNova={o=>{
     ...
     const vazio={caixa:{saldo:0,hist:[]},custos:[],acomp:[],estoque:[],consumo:[],msgs:[],compras:[],rec:[],emp:[],emprestimos:[],lidos:{}};
 ...
 const TA=["Dashboard","Orçamento","Custos","Recebimentos","Caixa","Empréstimo","Medições","Empreitada","Estoque","Compras","Comunicação"];
 ...
 {tab===6&&<Medicoes A={D.acomp||[]} sv={v=>sv("acomp",v)} obra={obra}/>}
 {tab===7&&<Empreitada E={D.emp||[]} sv={v=>sv("emp",v)} A={D.acomp||[]} svA={v=>sv("acomp",v)}/>}
 {tab===8&&<Estoque ES={D.estoque||[]} CON={D.consumo||[]} svE={v=>sv("estoque",v)} svC={v=>sv("consumo",v)}/>}
 {tab===9&&<Compras CP={D.compras||[]} sv={v=>sv("compras",v)} svE={v=>sv("estoque",v)} ES={D.estoque||[]}/>}
     {tab===10&&<Comunicacao C={D.custos||[]} sv={v=>sv("custos",v)} msgs={D.msgs||[]} svM={v=>sv("msgs",v)} user={user}/>}
 </>}
 {role==="eng"&&<>
...
function ObrasList({user,obras,onSel,onOut,onNova,onDel,onUpd}){
 ...
 const Card=o=>{
  ...
  const semanas=ini?Math.max(1,Math.floor((new Date().getTime()-ini.getTime())/(7*86400000))+1):null;
  return(
   <div key={o.id} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:14,padding:24,boxShadow:"0 1px 4px #0001",position:"relative",opacity:arq?0.75:1}}>
    <div onClick={()=>onSel(o.id)} style={{cursor:"pointer"}}>
     <div style={{display:"flex",justifyContent:"space-between",marginBottom:12,gap:8}}>
      ...
      <span style={{fontSize:10,color:"#64748b",textAlign:"right"}}>Início {o.inicio||"—"}{semanas&&!arq?" · S"+semanas:""}</span>
     </div>
    ...
    <Modal onClose={()=>setEdit(null)} title="Editar Obra">
     <div style={{display:"grid",gap:10}}>
      ...
      <div style={{background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:8,padding:"9px 12px",fontSize:11,color:"#1e40af",lineHeight:1.45}}>
       As semanas de medição (S1, S2, S3…) passam a ser contadas a partir desta data.
       {eForm.inicio&&parseBRDate(eForm.inicio)?" Hoje a obra está na semana S"+Math.max(1,Math.floor((new Date().getTime()-parseBRDate(eForm.inicio).getTime())/(7*86400000))+1)+".":""}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
...
function Medicoes({A,sv,obra,user,svM,msgs}){
 ...
 const semObra=getSemanaAtual();
 const semSugerida=Math.max(lastSem+1,SEM_BASE?semObra:3);
 ...
 return(
  <div>
   ...
   <div style={{fontSize:12,color:"#64748b",marginBottom:12}}>Próxima semana: <b style={{color:"#f59e0b"}}>{nextSemLabel}</b> (semana {nextSem}){SEM_BASE?<span> · contagem a partir do início da obra em <b>{inicioObra}</b></span>:<span style={{color:"#ef4444"}}> · informe a data de início da obra para as semanas baterem com o calendário</span>}</div>
    ...
    <Modal onClose={()=>setModal(false)} title={`Registrar Medição — ${nextSemLabel}`}>
     <div style={{background:"#fef3c7",borderRadius:8,padding:12,marginBottom:16,fontSize:12,color:"#92400e"}}>
      ...
      <div style={{marginTop:8,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
       <span style={{fontSize:11,color:"#64748b"}}>Semana da medição:</span>
...
function Emprestimos({EM,sv}){
 ...
 const [form,setForm]=useState({nome:"",instituicao:"",valorContratado:0,valorLiberado:0,valorTotal:0,parcelas:0,valorParcela:0,data:"",obs:"",noCaixa:false});
 ...
 const salvar=()=>{
  ...
  const base={nome:form.nome,instituicao:form.instituicao,valorContratado:contratado,valorLiberado:liberado,valorTotal,parcelas,valorParcela,data:form.data,obs:form.obs,noCaixa:!!form.noCaixa};
  if(modal==="edit"&&selId){
 ...
 return(
  <div>
   ...
   <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:12,marginBottom:20}}>
    ...
    <KCard label="Custo do Crédito" value={fmtBR(custoCredito)} color="#f59e0b" sub={descontos>0?"já com "+fmtBR(descontos)+" de desconto por antecipação":"juros e encargos (impacto no lucro)"}/>
   </div>
   {foraDoCaixa>0&&(
     ...
     <div key={e.id} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:16,marginBottom:16,boxShadow:"0 1px 4px #0001"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",gap:10,marginBottom:12}}>
       ...
       <div style={{background:"#f8fafc",borderRadius:8,padding:"9px 12px"}}>
        ...
        {juros>0&&<div style={{fontSize:10,color:"#94a3b8"}}>custo do crédito {fmtBR(juros)}</div>}
     ...
     <div style={{display:"grid",gap:10}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
       <label style={{fontSize:12,color:"#64748b"}}>Valor que entrou na conta (R$)
        <input type="number" value={form.valorLiberado} onChange={e=>setForm(f=>({...f,valorLiberado:e.target.value}))} placeholder="igual ao contratado" style={IS}/>
       </label>
