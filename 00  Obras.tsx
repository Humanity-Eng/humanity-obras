...
function semLabelDe(n,obra){
  ...
  if(obra&&obra.inicio)return semLabelObra(n,obra.inicio);
  return`S${n}`;
}
...
const OBRAS_SEED=[
 ...
 {id:"biomagistral",nome:"BIOMAGISTRAL — Reforma de Prédio Administrativo",cliente:"João Lucas da Silva Gonçalves",local:"Belém/PA",inicio:"27/07/2026",contrato:44000,status:"Em andamento",arquivada:false,concluida:false},
];
 ...
 useEffect(()=>{
  (async()=>{
   ...
   if(lista&&Array.isArray(lista)&&lista.length>0){
    ...
    const FIX={biomagistral:{cliente:"João Lucas da Silva Gonçalves",inicio:"27/07/2026"}};
      ...
      <div key={o.id} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:14,padding:24,boxShadow:"0 1px 4px #0001",position:"relative"}}>
       <div onClick={()=>onSel(o.id)} style={{cursor:"pointer"}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
         ...
         <span style={{fontSize:10,color:"#64748b"}}>{o.concluida&&o.fim?"Concluída em "+o.fim:o.inicio}</span>
        </div>
...
function Orcamento({obraId,obra}){
 ...
 const pavTot=p=>Object.values(TAB[p]).reduce((s,its)=>s+its.reduce((ss,i)=>ss+i.total,0),0);
