...
// ── Acesso ao banco via REST puro (fetch) ─────────────────────────────
// Antes usávamos a biblioteca supabase-js. No Safari/iOS ela dispara
// "DataCloneError" ao tentar sincronizar sessão entre abas/instâncias,
// o que fazia o sistema cair no cache local e mostrar dados velhos.
// A API REST do Supabase é HTTP simples: sem GoTrueClient, sem clone,
// sem erro. Mesmo banco, mesmos dados, mesma chave.
const REST=SUPA_URL+"/rest/v1/obra_data";
const HDRS={apikey:SUPA_KEY,Authorization:"Bearer "+SUPA_KEY,"Content-Type":"application/json"};

function lsGet(obraId,chave){
 try{
  const raw=localStorage.getItem(obraId+":"+chave);
  return raw?JSON.parse(raw):null;
 }catch(e){return null;}
}
function lsSet(obraId,chave,valor){
 try{localStorage.setItem(obraId+":"+chave,JSON.stringify(valor));}catch(e){}
}
// Indica se a última conversa com o banco funcionou — usado para avisar na tela
let _bancoOk=null;
function bancoOnline(){return _bancoOk;}

async function dbGet(obraId,chave){
 const url=REST+"?obra_id=eq."+encodeURIComponent(obraId)+"&chave=eq."+encodeURIComponent(chave)+"&select=valor&limit=1";
 try{
  const ctrl=new AbortController();
  const t=setTimeout(()=>ctrl.abort(),10000);
  const r=await fetch(url,{headers:HDRS,signal:ctrl.signal});
  clearTimeout(t);
  if(!r.ok){_bancoOk=false;console.warn("dbGet HTTP",r.status);return lsGet(obraId,chave);}
  const rows=await r.json();
  _bancoOk=true;
  if(Array.isArray(rows)&&rows.length>0&&rows[0].valor!==undefined){
   lsSet(obraId,chave,rows[0].valor); // mantém o cache local em dia
   return rows[0].valor;
  }
  return null; // não existe no banco ainda → deixa a semente entrar
 }catch(e){
  _bancoOk=false;
  console.warn("dbGet:",e&&e.message);
  return lsGet(obraId,chave);
 }
}

async function dbSet(obraId,chave,valor){
 lsSet(obraId,chave,valor); // grava local primeiro, para não perder nada
 try{
  const ctrl=new AbortController();
  const t=setTimeout(()=>ctrl.abort(),10000);
  const r=await fetch(REST+"?on_conflict=obra_id,chave",{
   method:"POST",
  ...
  clearTimeout(t);
  if(!r.ok){_bancoOk=false;console.warn("dbSet HTTP",r.status);}
  else _bancoOk=true;
 }catch(e){_bancoOk=false;console.warn("dbSet:",e&&e.message);}
}

const SAL="humanity-eng-2026";
async function hashSenha(email,senha){
 const txt=SAL+":"+String(email).toLowerCase().trim()+":"+senha;
 const buf=await crypto.subtle.digest("SHA-256",new TextEncoder().encode(txt));
 return Array.from(new Uint8Array(buf)).map(b=>b.toString(16).padStart(2,"0")).join("");
}
const USERS={
  "diretor@humanityengenharia.com":{pwh:"92b67636aff1a8a3886598ee558c563d4398255e19be1d04a03f3df06a202ba0",role:"admin",nome:"Hugo Puty",obras:"*"},
  "moniquepacheco.eng@outlook.com":{pwh:"6854b282aa9ce7ea228e3da2a507e7869af1c6b4c1851f4a1c581d452f0e7517",role:"eng",nome:"Monique Pacheco",obras:["lote_d18"]},
  "cliente@loted18":{pwh:"bec3a13aaef3309cdd8e7500685fabb1b2f2a125e8dc438a6d26e795c5b61f15",role:"cli",nome:"Marcelo Azevedo Costa",obras:["lote_d18"]},
};
...
// Rótulo legado da D18: tabela fixa ancorada em Jun/2026.
// Mantido intacto para não renomear as medições já registradas naquela obra.
function semLabelLegado(n){
...
function parseDataBR(str){
  // "27/07/2026" -> Date. Rejeita datas impossíveis: o construtor Date faz
  // rollover (31/02 vira 03/03), então conferimos se a data "voltou" igual.
...
// Semana N contada a partir do início da obra: devolve a data daquela semana
function dataDaSemana(inicio,n){
  const base=parseDataBR(inicio);
  if(!base)return null;
  const d=new Date(base.getTime());
  d.setDate(d.getDate()+(n-1)*7);
  return d;
}
// Rótulo real: "S4 (Jul)" onde S4 = 4ª semana daquele mês do calendário
function semLabelObra(n,inicio){
  const d=dataDaSemana(inicio,n);
  if(!d)return`S${n}`;
  const semanaDoMes=Math.floor((d.getDate()-1)/7)+1;
  return`S${semanaDoMes} (${MESES_BR[d.getMonth()]})`;
}
// Ponto único de decisão: a D18 mantém o rótulo antigo, as demais usam a data real
// Uma obra sem data de início definida ainda não tem calendário:
// mostramos apenas "Semana N" até que a data real seja preenchida.
function temInicio(obra){
  return !!(obra&&obra.inicio&&parseDataBR(obra.inicio));
}
function semLabelDe(n,obra){
  if(obra&&obra.id==="lote_d18")return semLabelLegado(n);
  if(temInicio(obra))return semLabelObra(n,obra.inicio);
  return`Semana ${n}`;
}
...
function calcConclusaoGeral(A,obraId){
  if(!A||A.length===0)return 0;
  const cfg=obraId?OBRA_CFG[obraId]:null;
  // Base = total de dias configurado da obra; se não houver, soma os prazos das atividades
  const base=(cfg&&cfg.totalDias)||A.reduce((s,a)=>s+(a.prazo||0),0);
  if(!base)return 0;
  return A.reduce((s,a)=>s+(a.total>0?(a.acum/a.total)*a.prazo:0),0)/base*100;
}
function totalDiasDe(A,obraId){
  const cfg=obraId?OBRA_CFG[obraId]:null;
  return (cfg&&cfg.totalDias)||(A||[]).reduce((s,a)=>s+(a.prazo||0),0);
}

// ═══════════ BIOMAGISTRAL — Reforma de prédio administrativo ═══════════
// Acompanhamento (peso em dias) — total 13,2 dias
const BIO_TOTAL_DIAS=13.2;
const BIO_ACOMP=[
["Demolição","Demolição","1.1","Demolição de Alvenaria",0.5,"M2",11.97,0],
["Demolição","Demolição","1.2","Remoção de Janela",0.5,"UND",10,0],
["Demolição","Demolição","1.3","Remoção de Porta",0.4,"UND",2,0],
["Parede","Parede","2.1","Parede de Placa Cimentícia",1,"M2",11.7,0],
["Parede","Parede","2.2","Parede de Placa de Gesso Acartonado",2,"M2",30.12,0],
["Pintura","Pintura","3.1","Aplicação de Selador",0.8,"M2",83.64,0],
["Pintura","Pintura","3.2","Emassamento",1.5,"M2",83.64,0],
["Pintura","Pintura","3.3","Pintura",1.5,"M2",83.64,0],
["Esquadria","Esquadria","4.1","Instalação de Porta MDF",1.6,"UND",8,0],
["Esquadria","Esquadria","4.2","Instalação de Porta de Alumínio",0.2,"UND",1,0],
["Esquadria","Esquadria","4.3","Instalação de Passthrougt",0.3,"UND",1,0],
["Esquadria","Esquadria","4.4","Instalação de Grelha em Porta",0.5,"UND",2,0],
["Acessórios","Acessórios","5.1","Instalação de Ventokit",0.4,"UND",2,0],
["Acessórios","Acessórios","5.2","Instalação de Insuflador de Ar",0.8,"UND",4,0],
["Acessórios","Acessórios","5.3","Instalação de Exaustor",1.2,"UND",6,0],
].map(([pav,sec,cod,ativ,prazo,und,total,acum])=>({pav,sec,cod,ativ,prazo:+prazo,und,total:+total,acum:+acum,meds:[],medsPendentes:[],peso_obra:+prazo/BIO_TOTAL_DIAS*100}));

// Custos da empresa por etapa — total R$ 24.456,00
const BIO_CUSTOS=[
["Serviços Preliminares","Serviços Preliminares","ART de Obra","UND",1,290],
["Serviços Preliminares","Serviços Preliminares","Processo ANVISA","VB",1,1500],
["Demolição","Demolição","Saco de Ráfia","UND",8,5],
["Parede","Parede","Placa Cimentícia","UND",12,90],
["Parede","Parede","Placa de Gesso","UND",38,40],
["Parede","Parede","Guia 70","UND",15,28],
["Parede","Parede","Montante 70","UND",30,24],
["Parede","Parede","Fita Telada","UND",2,25],
["Parede","Parede","Massa para Drywall","BALDE",2,35],
["Parede","Parede","Massa Cimentícia","BALDE",1,90],
["Parede","Parede","Parafusos","VB",1,200],
["Pintura","Pintura","Selador Acrílico","LATÃO",1,120],
["Pintura","Pintura","Massa Acrílica","BALDE",1,90],
["Pintura","Pintura","Massa PVA","BALDE",3,70],
["Pintura","Pintura","Lona Plástica","M",4,6],
["Pintura","Pintura","Lixa","UND",10,1.2],
["Pintura","Pintura","Kit Pintura","UND",1,60],
["Pintura","Pintura","Extensor","UND",1,20],
["Pintura","Pintura","Tinta","GALÃO",2,300],
["Pintura","Pintura","Fita Crepe","ROLO",2,15],
["Esquadrias","Esquadrias","Porta 80","UND",5,900],
["Esquadrias","Esquadrias","Porta 70","UND",1,800],
["Esquadrias","Esquadrias","Porta 80 PCD","UND",2,1500],
["Esquadrias","Esquadrias","Porta Alumínio","UND",1,1000],
["Esquadrias","Esquadrias","Passthrougt","UND",1,200],
["Esquadrias","Esquadrias","Instalação de Grelha","UND",2,250],
["Acessórios","Acessórios","Cabo 2,5mm","ROLO",1,290],
["Acessórios","Acessórios","Fita Isolante","ROLO",1,20],
["Acessórios","Acessórios","Acessórios Extras","VB",1,500],
["Mão de Obra","Mão de Obra","Gesseiro + Instalação de Portas","VB",1,4000],
["Mão de Obra","Mão de Obra","Pintor","VB",1,1200],
["Mão de Obra","Mão de Obra","Eletricista","VB",1,800],
["Mão de Obra","Mão de Obra","Servente","VB",1,500],
].map(([pav,cat,desc,und,qtd,unit])=>({pav,cat,desc,und,qtd:+qtd,unit:+unit,total:+qtd*+unit,pago:0,status:"pendente",forn:""}));

// Orçamento enviado ao cliente — total arredondado R$ 44.000,00 (BDI 18%)
const BIO_ORC={
 "Demolição":{"Demolição":[
  {cod:"97622",desc:"Demolição de alvenaria de bloco furado, de forma manual, sem reaproveitamento. AF_09/2023",und:"m2",qtd:11.97,unit:40,total:478.80},
  {cod:"-",desc:"Remoção de janela com reaproveitamento",und:"Und.",qtd:10,unit:50,total:500},
  {cod:"-",desc:"Remoção de porta com reaproveitamento",und:"Und.",qtd:2,unit:50,total:100},
 ]},
 "Parede":{"Parede":[
  {cod:"-",desc:"Fechamento duplo com placa cimentícia com reforço para instalação de móveis",und:"M2",qtd:11.7,unit:220,total:2574},
  {cod:"-",desc:"Parede drywall em placa de gesso acartonado",und:"M2",qtd:30.12,unit:140,total:4216.80},
 ]},
 "Pintura":{"Pintura":[
  {cod:"-",desc:"Fundo selador acrílico, aplicação manual em parede, uma demão. AF_04/2023",und:"M2",qtd:83.64,unit:15,total:1254.60},
  {cod:"-",desc:"Aplicação de massa acrílica em parede de ambiente interno e externo",und:"M2",qtd:83.64,unit:35,total:2927.40},
  {cod:"-",desc:"Aplicação de tinta acrílica fosco (cor à definir)",und:"M2",qtd:83.64,unit:45,total:3763.80},
 ]},
 "Esquadria":{"Esquadria":[
  {cod:"-",desc:"Porta de giro em madeira 80x210cm com pintura esmalte branca e maçaneta tipo alavanca",und:"Und.",qtd:5,unit:1700,total:8500},
  {cod:"-",desc:"Porta de giro em madeira 70x210cm com pintura esmalte branca e maçaneta tipo alavanca",und:"Und.",qtd:1,unit:1500,total:1500},
  {cod:"-",desc:"Porta de giro em madeira 80x210cm com pintura esmalte branca, maçaneta tipo alavanca e barras de apoio",und:"Und.",qtd:2,unit:2000,total:4000},
  {cod:"-",desc:"Porta de giro de alumínio",und:"Und.",qtd:1,unit:2000,total:2000},
  {cod:"-",desc:"Passthrougt em MDF branco TX e porta de giro",und:"Und.",qtd:1,unit:1000,total:1000},
  {cod:"-",desc:"Instalação de grelha 30x30cm na porta",und:"Und.",qtd:2,unit:500,total:1000},
 ]},
 "Acessórios":{"Acessórios":[
  {cod:"-",desc:"Instalação de ventokit (equipamento fornecido pelo cliente)",und:"Und.",qtd:2,unit:450,total:900},
  {cod:"-",desc:"Instalação de insuflamento de ar (equipamento fornecido pelo cliente)",und:"Und.",qtd:1,unit:550,total:550},
  {cod:"-",desc:"Instalação de exaustor de ar (equipamento fornecido pelo cliente)",und:"Und.",qtd:6,unit:450,total:2700},
 ]},
};

// Registro por obra: dados-semente, total de dias e metadados do orçamento
// ═══════════ CRTR14 — Reforma prédio administrativo (Conselho Reg. Téc. Radiologia) ═══════════
const CRTR_TOTAL_DIAS=50.2;
const CRTR_ACOMP=[
["Parede com Rachaduras","1.1","Demolição de Alvenaria","MÁRCIO",0.8,"M2",18],
["Parede com Rachaduras","1.2","Alvenaria","MÁRCIO",1.5,"M2",18],
["Parede com Rachaduras","1.3","Reboco","MÁRCIO",1,"M2",36],
["Parede com Rachaduras","1.4","Aplicação de Massa em Parede","EDMAURO",1,"M2",36],
["Parede com Rachaduras","1.5","Aplicação de Tinta em Parede","EDMAURO",0.5,"M2",36],
["Parede com Infiltração","2.1","Remoção de Reboco","MÁRCIO",0.3,"M2",24],
["Parede com Infiltração","2.2","Reboco","MÁRCIO",0.8,"M2",24],
["Parede com Infiltração","2.3","Aplicação de Massa em Parede","EDMAURO",0.8,"M2",24],
["Parede com Infiltração","2.4","Aplicação de Tinta em Parede","EDMAURO",0.5,"M2",24],
["Cobertura","3.1","Remoção de Cobertura","MÁRCIO",1.5,"M2",130],
["Cobertura","3.2","Ajuste de Madeiramento","MÁRCIO",2,"M2",130],
["Cobertura","3.3","Ajuste de Telhamento","MÁRCIO",2,"M2",130],
["Revitalização da Área Externa","4.1","Aplicação de Massa em Parede","EDMAURO",1.5,"M2",150],
["Revitalização da Área Externa","4.2","Aplicação de Tinta em Parede","EDMAURO",1.5,"M2",150],
["Revitalização da Área Externa","4.3","Aplicação de Tinta em Piso","EDMAURO",1,"M2",40],
["Revitalização da Área Externa","4.4","Cobertura Completa","MÁRCIO",1,"M2",3],
["Piso","5.1","Demolição de Revestimento Cerâmico","MÁRCIO",1.5,"M2",130],
["Piso","5.2","Assentamento de Piso Cerâmico","MÁRCIO",5,"M2",130],
["Piso","5.3","Rodapé","DAVID",2,"M2",100],
["Piso","5.4","Aplicação de Rejunte","MÁRCIO",1,"M2",130],
["Elétrica","6.1","Ponto de Tomada","ALLAN",1,"UND",15],
["Elétrica","6.2","Ponto de Iluminação","ALLAN",1,"UND",15],
["Elétrica","6.3","Instalação de Luminária","ALLAN",0.5,"UND",15],
["Forro","7.1","Forro de Gesso Acartonado","DAVID",4,"M2",130],
["Esquadria","8.1","Porta HDF","DAVID",2.5,"UND",10],
["Esquadria","8.2","Pele de Vidro","GRANVIMAR",2.5,"M2",30],
["Pintura","9.1","Aplicação de Massa em Teto","EDMAURO",2,"M2",130],
["Pintura","9.2","Aplicação de Tinta em Teto","EDMAURO",2,"M2",130],
["Pintura","9.3","Aplicação de Massa em Parede","EDMAURO",3,"M2",220],
["Pintura","9.4","Aplicação de Tinta em Parede","EDMAURO",3,"M2",220],
["Fachada - Ajustes","10.1","Alvenaria","MÁRCIO + EDMAURO",1.5,"VB",1],
].map(([pav,cod,ativ,resp,prazo,und,total])=>({pav,sec:pav,cod,ativ,resp,prazo:+prazo,und,total:+total,acum:0,meds:[],medsPendentes:[],peso_obra:+prazo/CRTR_TOTAL_DIAS*100}));

// Custos da empresa — total R$ 174.329,40
const CRTR_CUSTOS=[
["Serviços Preliminares","Container de Entulho","UND",4,350],
["Serviços Preliminares","ART de Obra","UND",1,290],
["Parede com Rachaduras","Tijolo","UND",720,0.85],
["Parede com Rachaduras","Areia","M3",1,130],
["Parede com Rachaduras","Cimento","SACA",5,51],
["Parede com Rachaduras","Fundo Selador","GALÃO",1,100],
["Parede com Rachaduras","Massa PVA","BALDE",1,70],
["Parede com Rachaduras","Lixa","UND",2,1.2],
["Parede com Rachaduras","Tinta à Definir","GALÃO",1,120],
["Parede com Rachaduras","Lona 4m","M",3,10],
["Parede com Infiltrações","Areia","M3",1,130],
["Parede com Infiltrações","Cimento","SACA",4,51],
["Parede com Infiltrações","Tinta à Definir","GALÃO",1,120],
["Parede com Infiltrações","Massa PVA","BALDE",1,70],
["Parede com Infiltrações","Selador Acrílico","GALÃO",1,100],
["Cobertura","Madeiramento","VB",1,2500],
["Cobertura","Telha de Fibrocimento","UND",130,25],
["Revitalização da Área Externa","Selador Acrílico","LATÃO",2,150],
["Revitalização da Área Externa","Massa Acrílica","BALDE",4,120],
["Revitalização da Área Externa","Tinta Sol e Chuva","LATÃO",2,400],
["Revitalização da Área Externa","Tinta Piso","LATÃO",1,150],
["Revitalização da Área Externa","Lixa N100","UND",10,1.2],
["Revitalização da Área Externa","Madeiramento","VB",1,300],
["Revitalização da Área Externa","Telha de Fibrocimento","UND",3,25],
["Demais Melhorias","Revestimento Cerâmico 60x60","M2",146,55],
["Demais Melhorias","Argamassa ACII","SACA",50,35],
["Demais Melhorias","Espaçador","PACOTE",15,15],
["Demais Melhorias","Rejunte","KG",15,20],
["Demais Melhorias","Rodapé de Poliestireno","M",100,18],
["Demais Melhorias","Cola para Rodapé","BALDE",5,90],
["Demais Melhorias","Luminária Quadrada 30x30cm","UND",15,30],
["Demais Melhorias","Cabo 1,5mm","UND",2,190],
["Demais Melhorias","Cabo 2,5mm","UND",2,290],
["Demais Melhorias","Tomadas e Interruptores","UND",30,20],
["Forro","Placa de Gesso 0,60x2,00","PLACAS",120,22],
["Forro","Fita Telada","ROLO",4,90],
["Forro","Massa para Gesso","BALDE",4,120],
["Forro","Parafuso GN25 Ponta Agulha","CAIXA",4,130],
["Forro","Demais Acessórios","VB",1,700],
["Forro","Massa PVA","BALDE",7,70],
["Forro","Tinta Branco Neve Fosco","LATÃO",2,150],
["Forro","Lixa","UND",10,1.2],
["Pintura","Lixa","UND",10,1.2],
["Pintura","Massa","BALDE",3,70],
["Pintura","Tinta à Definir","LATÃO",2,300],
["Pintura","Lona 4m","M",10,10],
["Esquadrias","Pele de Vidro","M2",30,1400],
["Esquadrias","Porta HDF Completa","UND",10,750],
["Esquadrias","Espuma Expansiva","UND",20,35],
["Mão de Obra e Aluguéis","Pedreiro","VB",1,17000],
["Mão de Obra e Aluguéis","Pintor","VB",1,5500],
["Mão de Obra e Aluguéis","Eletricista","VB",1,2340],
["Mão de Obra e Aluguéis","Instalador de Porta","VB",1,200],
["Mão de Obra e Aluguéis","Gesseiro - Forro","VB",1,4600],
["Não Previsto + Imposto","DAS - Imposto","UND",1,28500],
["Não Previsto + Imposto","Repasse","UND",1,30500],
["Não Previsto + Imposto","Ajustes Fachada","VB",1,3000],
].map(([pav,desc,und,qtd,unit])=>({pav,cat:pav,desc,und,qtd:+qtd,unit:+unit,total:+qtd*+unit,pago:0,status:"pendente",forn:""}));

// Orçamento ao cliente — R$ 202.582,19 (BDI 18% + NF 15% por bloco)
const CRTR_ORC={
 "Parede com Rachaduras":{"Demolição":[
  ...
  {cod:"103357",desc:"Alvenaria de vedação de blocos cerâmicos furados na horizontal de 9x19x29cm (espessura 9cm) e argamassa de assentamento com preparo manual. AF_12/2021",und:"M2",qtd:18,unit:115,total:2070},
  ...
  {cod:"-",desc:"Aplicação de tinta acrílica fosco (cor à definir)",und:"M2",qtd:36,unit:44,total:1584},
 ]},
 "Parede Infiltrações":{"Demolição":[
  ...
  {cod:"-",desc:"Aplicação de tinta acrílica fosco (cor à definir)",und:"M2",qtd:24,unit:44,total:1056},
 ],"Cobertura":[
  {cod:"-",desc:"Revisão completa de telhado (com trocas onde for necessário)",und:"M2",qtd:130,unit:165,total:21450},
 ]},
 "Revitalização de Área Externa":{"Pintura":[
 ...
 "Melhorias em Geral":{"Troca de Piso":[
  ...
  {cod:"-",desc:"Revestimento cerâmico para piso com placas tipo esmaltada de dimensões 60x60cm aplicada em ambientes de área maior que 10m2. AF_02/2023_PE",und:"M2",qtd:130,unit:115,total:14950},
  ...
  {cod:"-",desc:"Aplicação de tinta acrílica fosco (cor à definir)",und:"Und.",qtd:220,unit:44,total:9680},
 ],"Fachada em Pele de Vidro":[
  {cod:"-",desc:"Pele de vidro",und:"M2",qtd:30,unit:1650,total:49500},
  {cod:"-",desc:"Porta HDF completa (listada no orçamento, fora do subtotal do bloco)",und:"Und.",qtd:10,unit:1200,total:12000},
 ]},
};
// Totais finais por bloco, conforme o PDF (subtotal + BDI 18% + NF 15%).
// Fixos porque o bloco "Melhorias em Geral" exclui a Porta HDF do subtotal.
const CRTR_ORC_TOTAIS={
 "Parede com Rachaduras":11519.30,
 "Parede Infiltrações":33536.90,
 "Revitalização de Área Externa":11649.85,
 "Melhorias em Geral":145876.14,
};

const OBRA_CFG={
 lote_d18:{totalDias:TOTAL_DIAS_OBRA,orc:null,orcInfo:"Proposta 060/2025 · SINAPI 11/2023 · BDI 12%",
  pavs:["Fundação","Térreo","1º Pavimento","Cobertura","Fachada"]},
 biomagistral:{totalDias:BIO_TOTAL_DIAS,orc:BIO_ORC,orcInfo:"Proposta 102/2026 · SINAPI 11/2023 · BDI 18%",
  pavs:["Demolição","Parede","Pintura","Esquadria","Acessórios"]},
 crtr14:{totalDias:CRTR_TOTAL_DIAS,orc:CRTR_ORC,orcTotais:CRTR_ORC_TOTAIS,
  orcInfo:"Proposta 005/2026 · SINAPI 11/2023 · BDI 18% + NF 15%",
  pavs:["Parede com Rachaduras","Parede com Infiltração","Cobertura","Revitalização da Área Externa","Piso","Elétrica","Forro","Esquadria","Pintura","Fachada - Ajustes"]},
};
const PAV_COLORS=["#f59e0b","#3b82f6","#10b981","#8b5cf6","#ef4444","#f97316","#06b6d4","#ec4899"];
// Etapas derivadas dos dados reais da obra (sem hardcode de pavimento)
function pavsDe(A,obraId){
 const doDado=[...new Set((A||[]).map(a=>a.pav))];
 if(doDado.length>0)return doDado;
 const cfg=OBRA_CFG[obraId];
 return cfg?cfg.pavs:[];
}
function coresDe(pavs){
 const m={};pavs.forEach((p,i)=>{m[p]=PAV_COLORS[i%PAV_COLORS.length];});return m;
}
...
const OBRAS_SEED=[
 {id:"lote_d18",nome:"LOTE D18 — Quinta das Orquídeas",cliente:"Marcelo Azevedo Costa",local:"Ananindeua/PA",inicio:"29/04/2026",contrato:750000,status:"Em andamento",arquivada:false,concluida:false},
 {id:"biomagistral",nome:"BIOMAGISTRAL — Reforma de Prédio Administrativo",cliente:"João Lucas da Silva Gonçalves",local:"Belém/PA",inicio:"",contrato:44000,status:"Em andamento",arquivada:false,concluida:false},
 {id:"crtr14",nome:"CRTR14 — Reforma de Prédio Administrativo",cliente:"Conselho Regional dos Técnicos em Radiologia 14ª região",local:"Belém/PA",inicio:"",contrato:202582.19,status:"Em andamento",arquivada:false,concluida:false},
];
...
function DashboardEng({D,obra}){
 ...
 const conclusao=A.length>0?calcConclusaoGeral(A,obraId):0;
 const pavs=pavsDe(A,obraId);
 const COLORS=coresDe(pavs);
...
export default function App(){
 ...
 // Sementes por obra: só acompanhamento, custos e orçamento vêm preenchidos.
 // Compras, estoque, empreitadas e recebimentos ficam vazios para preenchimento posterior.
 const SEED={
  lote_d18:{acomp:IA,custos:IC,estoque:IES,rec:IR,emp:IE_DEFAULT},
  biomagistral:{acomp:BIO_ACOMP,custos:BIO_CUSTOS},
  crtr14:{acomp:CRTR_ACOMP,custos:CRTR_CUSTOS},
 };
 const seed=SEED[obraId]||{};

 // ── Reset único das medições de teste da BIOMAGISTRAL ──────────────────
 // Roda uma vez só: grava um marcador no banco e nunca mais repete, para
 // não apagar medições reais registradas daqui em diante.
 // Restrito a "biomagistral" — a D18 jamais entra nesta rotina.
 const RESET_TAG="reset_medicoes_teste_v1";
 if(obraId==="biomagistral"){
  const jaResetou=await dbGet(obraId,RESET_TAG).catch(()=>null);
  if(!jaResetou){
   const limpo=BIO_ACOMP.map(a=>({...a,acum:0,meds:[],medsPendentes:[]}));
   await dbSet(obraId,"acomp",limpo);
   await dbSet(obraId,RESET_TAG,{feito:true,em:new Date().toISOString()});
  }
 }

 const nd={};
 const chaves=Object.keys(defs);
 // Busca todas as chaves de uma vez; se alguma falhar, vira null e cai na semente
 const vals=await Promise.all(chaves.map(k=>dbGet(obraId,k).catch(()=>null)));
 for(let ki=0;ki<chaves.length;ki++){
     const k=chaves[ki];
     const v=vals[ki];
     const base=seed[k];
     if(k==="acomp"&&base&&v&&Array.isArray(v)&&v.length>0){
      // Mescla a lista de atividades de referência preservando o progresso já medido
      const saved=v;
      ...
      // Primeira carga desta obra: grava a semente para virar dado editável
      nd[k]=base;
      dbSet(obraId,k,base);
     } else {
      nd[k]=Array.isArray(defs[k])?[]:defs[k];
     }
    }
 setD(nd);setLoading(false);
 }catch(e){
  // Nunca deixar a tela presa em "Carregando..."
  console.warn("carga da obra falhou:",e&&e.message);
  setD({});setLoading(false);
 }
 })();
 },[obraId]);
 useEffect(()=>{
  (async()=>{
   const lista=await dbGet("_sistema","obras");
   if(lista&&Array.isArray(lista)&&lista.length>0){
    // Preserva o que já está salvo e apenas acrescenta obras do seed ainda ausentes
    const faltando=OBRAS_SEED.filter(sd=>!lista.some(o=>o.id===sd.id));
    // Correções de cadastro aplicadas a obras já salvas. A D18 nunca entra aqui:
    // seus dados permanecem exatamente como estão no banco.
    const FIX={biomagistral:{cliente:"João Lucas da Silva Gonçalves"}};
    let mudou=faltando.length>0;
    const corrigidas=lista.map(o=>{
     const f=FIX[o.id];
     if(!f)return o;
     const dif=Object.keys(f).some(k=>o[k]!==f[k]);
     if(dif)mudou=true;
     return dif?{...o,...f}:o;
    });
    const merged=[...corrigidas,...faltando].map(o=>({arquivada:false,concluida:false,...o}));
    setObras(merged);
    if(mudou)dbSet("_sistema","obras",merged);
   } else {
    setObras(OBRAS_SEED);
    dbSet("_sistema","obras",OBRAS_SEED);
   }
  })();
 },[]);
 useEffect(()=>{
 ...
 if(!obraId)return <ObrasList user={user} obras={obras} onSel={id=>{setObraId(id);setTab(0);}} onOut={()=>setUser(null)}
    onNova={o=>{
     ...
     const vazio={custos:[],acomp:[],estoque:[],consumo:[],msgs:[],compras:[],rec:[],emp:[]};
    ...
    onUpd={(id,patch)=>{
     const n=obras.map(o=>o.id===id?{...o,...patch}:o);setObras(n);dbSet("_sistema","obras",n);
    }}/>;
 ...
 {tab===4&&<Medicoes A={D.acomp||[]} sv={v=>sv("acomp",v)} obra={obra} user={user} svM={v=>sv("msgs",v)} msgs={D.msgs||[]}/>}
 {tab===5&&<Empreitada E={D.emp||[]} sv={v=>sv("emp",v)} A={D.acomp||[]} svA={v=>sv("acomp",v)} obra={obra}/>}
 ...
 {role==="cli"&&<>
 {tab===0&&<CliAvancoObra A={D.acomp||[]} obra={obra}/>}
...
function Login({onLogin}){
 ...
 const go=async()=>{
  if(checando)return;
  const mail=em.toLowerCase().trim();
  const u=USERS[mail];
  setChecando(true);
  try{
   // Compara o hash do que foi digitado com o hash guardado.
   // A senha real nunca existe no código publicado.
   const h=await hashSenha(mail,pw);
   if(u&&u.pwh===h){const{pwh,...dados}=u;onLogin({...dados,email:em});}
   else setErr("E-mail ou senha incorretos.");
  }catch(e){
   setErr("Não foi possível validar o acesso. Tente novamente.");
  }
  setChecando(false);
 };
 return(
 ...
 <button onClick={go} disabled={checando} style={{...BP,width:"100%",marginTop:8,padding:"12px",opacity:checando?.6:1,cursor:checando?"default":"pointer"}}>{checando?"Verificando...":"Entrar"}</button>
 </div>
 </div>
 );
}
function ObrasList({user,obras,onSel,onOut,onNova,onDel,onUpd}){
 ...
 const [dataObra,setDataObra]=useState(null);   // id da obra cuja data está sendo editada
 const [dataVal,setDataVal]=useState("");
 const [form,setForm]=useState({nome:"",cliente:"",local:"",inicio:"",contrato:0});
 const permitidas=user.obras==="*"?obras:obras.filter(o=>(user.obras||[]).indexOf(o.id)>=0);
 const ativas=permitidas.filter(o=>!o.arquivada&&!o.concluida);
 const arquivadas=permitidas.filter(o=>o.arquivada&&!o.concluida);
 const concluidas=permitidas.filter(o=>o.concluida);
 const visiveis=aba==="arquivadas"?arquivadas:aba==="concluidas"?concluidas:ativas;
 const ABAS=[["ativas","Em Andamento",ativas.length],["arquivadas","Arquivadas",arquivadas.length],["concluidas","Concluídas",concluidas.length]];
 const criar=()=>{
  if(!form.nome.trim()){alert("Informe o nome da obra.");return;}
  const id="obra_"+Date.now();
  onNova({id,nome:form.nome,cliente:form.cliente||"—",local:form.local||"—",inicio:form.inicio||new Date().toLocaleDateString("pt-BR"),contrato:Number(form.contrato)||0,status:"Em andamento",arquivada:false,concluida:false});
 ...
 return(
  <div style={{minHeight:"100vh",background:"#f8fafc",fontFamily:"'Inter',system-ui,sans-serif"}}>
   ...
   <div style={{maxWidth:1280,margin:"0 auto",padding:"32px 16px"}}>
    {bancoOnline()===false&&(
     <div style={{background:"#fef3c7",border:"1px solid #fcd34d",color:"#92400e",borderRadius:10,padding:"10px 14px",marginBottom:16,fontSize:12,fontWeight:600}}>
      ⚠️ Sem conexão com o banco — exibindo dados salvos neste aparelho, que podem estar desatualizados.
     </div>
    )}
    <h2 style={{fontSize:22,fontWeight:800,color:"#f59e0b",marginBottom:4}}>Obras</h2>
    <p style={{color:"#64748b",marginBottom:18}}>Selecione uma obra para acessar o painel completo.</p>
    <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:24}}>
     {ABAS.map(([k,rot,qtd])=>(
      <button key={k} onClick={()=>setAba(k)} style={{background:aba===k?"#f8c400":"#fff",color:aba===k?"#0f172a":"#64748b",border:"1px solid #e2e8f0",borderRadius:20,padding:"7px 16px",fontSize:12,fontWeight:700,cursor:"pointer"}}>
       {rot} <span style={{opacity:.7}}>({qtd})</span>
      </button>
     ))}
    </div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:16}}>
     {visiveis.map(o=>(
      <div key={o.id} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:14,padding:24,boxShadow:"0 1px 4px #0001",position:"relative"}}>
       <div onClick={()=>onSel(o.id)} style={{cursor:"pointer"}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
         <span style={{fontSize:10,fontWeight:700,...(o.concluida?{color:"#16a34a",background:"#dcfce7"}:o.arquivada?{color:"#64748b",background:"#f1f5f9"}:{color:"#f59e0b",background:"#fef3c7"}),padding:"3px 10px",borderRadius:20}}>{(o.status||"").toUpperCase()}</span>
         <span style={{fontSize:10,color:o.inicio?"#64748b":"#f59e0b",fontWeight:o.inicio?400:700}}>{o.concluida&&o.fim?"Concluída em "+o.fim:(o.inicio||"início à definir")}</span>
        </div>
       ...
       {user.role==="admin"&&(
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:14,paddingTop:12,borderTop:"1px solid #f1f5f9"}}>
         {!o.concluida&&(
          o.arquivada
          ? <button onClick={()=>onUpd(o.id,{arquivada:false,status:"Em andamento"})} style={{background:"#dcfce7",border:"none",color:"#16a34a",borderRadius:6,padding:"5px 10px",cursor:"pointer",fontSize:11,fontWeight:700}}>Desarquivar</button>
          : <button onClick={()=>onUpd(o.id,{arquivada:true,status:"Arquivada"})} style={{background:"#f1f5f9",border:"none",color:"#64748b",borderRadius:6,padding:"5px 10px",cursor:"pointer",fontSize:11,fontWeight:700}}>Arquivar</button>
         )}
         {o.concluida
          ? <button onClick={()=>onUpd(o.id,{concluida:false,status:"Em andamento",fim:""})} style={{background:"#dbeafe",border:"none",color:"#1e40af",borderRadius:6,padding:"5px 10px",cursor:"pointer",fontSize:11,fontWeight:700}}>Reabrir</button>
          : <button onClick={()=>{if(confirm("Marcar a obra \""+o.nome+"\" como concluída?"))onUpd(o.id,{concluida:true,arquivada:false,status:"Concluída",fim:new Date().toLocaleDateString("pt-BR")});}} style={{background:"#dcfce7",border:"none",color:"#16a34a",borderRadius:6,padding:"5px 10px",cursor:"pointer",fontSize:11,fontWeight:700}}>Concluir</button>
         }
         <button onClick={()=>{setDataObra(o.id);setDataVal(o.inicio||"");}} style={{background:"#dbeafe",border:"none",color:"#1e40af",borderRadius:6,padding:"5px 10px",fontSize:11,fontWeight:700,cursor:"pointer"}}>{o.inicio?"Alterar início":"Definir início"}</button>
         {o.id!=="lote_d18"&&(
     ...
     {visiveis.length===0&&(
      <div style={{gridColumn:"1/-1",textAlign:"center",padding:"48px 20px",color:"#94a3b8"}}>
       <div style={{fontSize:34,marginBottom:10}}>{aba==="arquivadas"?"📦":aba==="concluidas"?"✅":"🏗️"}</div>
       <div style={{fontSize:14,fontWeight:700,color:"#64748b"}}>
        {aba==="arquivadas"?"Nenhuma obra arquivada":aba==="concluidas"?"Nenhuma obra concluída":"Nenhuma obra em andamento"}
       </div>
      </div>
     )}
     {user.role==="admin"&&aba==="ativas"&&(
   ...
   {dataObra&&(
    <Modal onClose={()=>setDataObra(null)} title="Data de início da obra">
     <p style={{fontSize:12,color:"#94a3b8",margin:"0 0 14px"}}>
      As medições semanais são contadas a partir desta data: a Semana 1 passa a ser a semana do início da obra.
      Deixe em branco enquanto a data ainda não estiver definida.
     </p>
     <label style={{fontSize:12,color:"#64748b"}}>Início (dd/mm/aaaa)
      <input value={dataVal} onChange={e=>setDataVal(e.target.value)} placeholder="27/07/2026" style={{...IS,marginTop:4}}/>
     </label>
     {dataVal&&!parseDataBR(dataVal)&&(
      <div style={{fontSize:12,color:"#ef4444",marginTop:8,fontWeight:600}}>Data inválida — use o formato dd/mm/aaaa.</div>
     )}
     {dataVal&&parseDataBR(dataVal)&&(
      <div style={{background:"#eff6ff",borderRadius:8,padding:10,marginTop:10,fontSize:12,color:"#1e40af"}}>
       A Semana 1 ficará como <b>{semLabelObra(1,dataVal)}</b>.
      </div>
     )}
     <div style={{display:"flex",gap:8,marginTop:16}}>
      <button onClick={()=>{
        if(dataVal&&!parseDataBR(dataVal))return;
        onUpd(dataObra,{inicio:dataVal.trim()});
        setDataObra(null);
      }} style={{...BP,flex:1}}>Salvar</button>
      <button onClick={()=>setDataObra(null)} style={{...BS,flex:1}}>Cancelar</button>
     </div>
    </Modal>
   )}
   {modal&&(
    <Modal onClose={()=>setModal(false)} title="Nova Obra">
     ...
     <div style={{display:"grid",gap:10}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
       ...
       <label style={{fontSize:12,color:"#64748b"}}>Início
...
function Dashboard({D,obra}){
 ...
 const obraId=obra?obra.id:null;
 const conclusao=calcConclusaoGeral(A,obraId);
 const totalDias=totalDiasDe(A,obraId);
 ...
 const pavCosts=[...new Set(C.map(c=>c.pav||"Outros"))].map(pav=>({
 ...
 return(
  <div>
   ...
   <p style={{fontSize:13,color:"#64748b",marginBottom:20}}>Início: {(obra&&obra.inicio)||"à definir"} · Cliente: {(obra&&obra.cliente)}</p>
   <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12,marginBottom:24}}>
    ...
    <KCard label="Lucro Previsto" value={fmtBR((obra?obra.contrato:0)-tC)} color="#10b981" sub={pct(((obra?obra.contrato:0)-tC)/(obra&&obra.contrato?obra.contrato:1)*100)+" de margem"}/>
   ...
   <div style={{background:"#fff",borderRadius:12,padding:20,marginBottom:20,boxShadow:"0 1px 4px #0001",border:"1px solid #e2e8f0"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
     <div><div style={{fontWeight:800,fontSize:16,color:"#1e293b"}}>Conclusão Total da Obra</div><div style={{fontSize:12,color:"#64748b"}}>Média ponderada por dias de atividade ({Number(totalDias||0).toLocaleString("pt-BR")} dias totais)</div></div>
...
function Orcamento({obraId,obra}){
 const cfg=OBRA_CFG[obraId];
 const TAB=obraId==="lote_d18"?ORC:(cfg&&cfg.orc);
 const temOrc=!!TAB;
 ...
 const pavs=Object.keys(TAB);
 const selAtual=(sel&&TAB[sel])?sel:pavs[0];
 // Soma bruta dos itens do bloco (antes de BDI/NF)
 const pavSub=p=>Object.values(TAB[p]).reduce((s,its)=>s+its.reduce((ss,i)=>ss+i.total,0),0);
 // Se a obra tem totais oficiais por bloco (com BDI/NF já aplicados), usamos eles.
 const TOTAIS=cfg&&cfg.orcTotais;
 const pavTot=p=>(TOTAIS&&TOTAIS[p]!==undefined)?TOTAIS[p]:pavSub(p);
  ...
  <div>
   <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16,flexWrap:"wrap",gap:8}}>
    <div>
     ...
     <p style={{fontSize:12,color:"#94a3b8",margin:"4px 0 0"}}>{(cfg&&cfg.orcInfo)||""}</p>
    </div>
    <div style={{fontSize:20,fontWeight:800,color:"#f59e0b"}}>{fmtBR(obra?obra.contrato:0)}</div>
   </div>
   <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:20}}>
    {pavs.map(p=><button key={p} onClick={()=>setSel(p)} style={{background:selAtual===p?"#f8c400":"#fff",color:selAtual===p?"#0f172a":"#64748b",border:"1px solid #e2e8f0",borderRadius:20,padding:"6px 14px",fontSize:12,fontWeight:700,cursor:"pointer"}}>
     {p} <span style={{opacity:.7,fontWeight:400}}>({fmtBR(pavTot(p))})</span>
    </button>)}
   </div>
   {Object.entries(TAB[selAtual]||{}).map(([sec,items])=>{
...
function Medicoes({A,sv,obra,user,svM,msgs}){
 ...
 const obraId=obra?obra.id:null;
 const pavs=pavsDe(A,obraId);
 const sel=(selRaw&&pavs.indexOf(selRaw)>=0)?selRaw:pavs[0];

 // Próxima semana = maior semana já medida + 1. A D18 tem 2 semanas de histórico anterior ao sistema.
 const lastSem=A.reduce((mx,a)=>Math.max(mx,(a.meds||[]).reduce((m,med)=>Math.max(m,med.sem||0),0)),0);
 const nextSem=obraId==="lote_d18"?Math.max(lastSem+1,3):lastSem+1;
 const nextSemLabel=semLabelDe(nextSem,obra);
 ...
 const getRepData=(sem)=>{
  ...
  const concGeral=calcConclusaoGeral(A,obraId);
  ...
  const label=semLabelDe(sem,obra);
 ...
 // Apaga TODAS as medições de uma semana, revertendo o acumulado de cada atividade
 const delSemana=(sem)=>{
  const rot=semLabelDe(sem,obra);
  if(!confirm("Apagar todas as medições de "+rot+"? O avanço volta ao valor anterior."))return;
  const n=A.map(a=>{
   const meds=a.meds||[];
   const remover=meds.filter(m=>m.sem===sem);
   if(remover.length===0)return a;
   const qtd=remover.reduce((t,m)=>t+(Number(m.qtd)||0),0);
   return{...a,meds:meds.filter(m=>m.sem!==sem),acum:Math.max((a.acum||0)-qtd,0)};
  });
  sv(n);
 };
 const delMed=()=>{
   ...
   {isEng&&totalPendentes>0&&(
    ...
    <div style={{background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:10,padding:"10px 14px",marginBottom:16}}>
     ...
     <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
      {semanasMedidas.map(sm=><button key={sm} onClick={()=>setRepSem(sm)} style={{background:"#1e40af",color:"#fff",border:"none",borderRadius:8,padding:"6px 12px",fontSize:12,fontWeight:700,cursor:"pointer"}}>{semLabelDe(sm,obra)}</button>)}
     </div>
     {!isEng&&(
      <div style={{marginTop:10,paddingTop:10,borderTop:"1px solid #bfdbfe"}}>
       <div style={{fontSize:11,color:"#64748b",marginBottom:6}}>Apagar uma medição inteira:</div>
       <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
        {semanasMedidas.map(sm=><button key={"d"+sm} onClick={()=>delSemana(sm)} style={{background:"#fee2e2",color:"#ef4444",border:"none",borderRadius:8,padding:"5px 10px",fontSize:11,fontWeight:700,cursor:"pointer"}}>🗑 {semLabelDe(sm,obra)}</button>)}
       </div>
      </div>
     )}
    </div>
     ...
     <div id="relatorio-cliente" style={{background:"#fff"}}>
      <div style={{borderBottom:"3px solid #f8c400",paddingBottom:14,marginBottom:16}}>
       ...
       <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"4px 16px",marginTop:10,fontSize:12}}>
        ...
        <div><span style={{color:"#94a3b8"}}>Início: </span><b style={{color:"#1e293b"}}>{(obra&&obra.inicio)||"à definir"}</b></div>
...
function Empreitada({E,sv,A,svA,obra}){
 ...
 e.medicoes=[...(e.medicoes||[]),{semana:newSem,label:semLabelDe(newSem,obra),itens:medItems,obs:formObs}];
 n[ei]=e;sv(n);
 ...
 meds:[...(nA[ai].meds||[]),{sem:newSem,label:semLabelDe(newSem,obra),qtd:mi.qtd,obs:"(Empreitada)",data:new Date().toLocaleDateString("pt-BR")}]};
 });
 ...
 return(
 <Modal onClose={()=>setModal(false)} title={`Medição ${semLabelDe(newSem,obra)} — ${e.nome}`}>
 <div style={{background:"#fef3c7",borderRadius:8,padding:10,marginBottom:12,fontSize:12,color:"#92400e"}}><b>{semLabelDe(newSem,obra)}</b> — Semana {newSem}</div>
...
function CliAvancoObra({A,obra}){
 ...
 const totalPct=calcConclusaoGeral(A,obraId);
 return(
 <div>
 <h2 style={{fontSize:20,fontWeight:800,color:"#f59e0b",marginBottom:4}}>Avanço da Obra</h2>
 <p style={{fontSize:13,color:"#64748b",marginBottom:20}}>{obra?obra.nome:"—"}{obra&&obra.local?" · "+obra.local:""}</p>
