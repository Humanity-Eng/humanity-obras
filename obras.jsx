...
async function dbSet(obraId,chave,valor){
 try{
  await sbReq("POST","/rest/v1/obra_data",{obra_id:obraId,chave,valor});
 }catch{
  try{await sbReq("PATCH","/rest/v1/obra_data?obra_id=eq."+encodeURIComponent(obraId)+"&chave=eq."+encodeURIComponent(chave),{valor});}
  catch(e){console.warn("dbSet:",e.message);}
 }
}
...
export default function App(){
 ...
 <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",height:52,gap:8}}>
  ...
  <div style={{flex:1,minWidth:0,padding:"0 8px"}}>
   <div style={{fontSize:12,fontWeight:700,color:"#f59e0b",lineHeight:1.2,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{obra?.nome}</div>
   <div style={{fontSize:10,color:"#64748b",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{obra?.local||"Ananindeua/PA"}</div>
  </div>
...
function Dashboard({D,obra}){
 ...
 return(
  <div>
   ...
   <p style={{fontSize:13,color:"#64748b",marginBottom:20}}>Início: {obra?.inicio} · Cliente: {obra?.cliente}</p>
...
function Medicoes({A,sv,obra,user,svM,msgs}){
 ...
 return(
  <div>
   ...
   {editTotal!==null&&(
    <Modal onClose={()=>setEditTotal(null)} title={`Editar — ${A[editTotal]?.ativ||""}`}>
     ...
     <p style={{fontSize:11,color:"#94a3b8",margin:"10px 0 0"}}>Acumulado atual: {A[editTotal]?.acum} {A[editTotal]?.und}. Você pode mudar a unidade de medição (ex: de M3 para UND) e a quantidade total conforme a obra avança.</p>
     <div style={{display:"flex",gap:10,marginTop:20}}>
...
function Empreitada({E,sv,A,svA}){
 ...
 const nA=[...A];
 medItems.forEach(mi=>{
 const ativName=(e.itens||[])[mi.ativIdx]?.ativ;if(!ativName)return;
 ...
 {modal==="pag"&&(
 <Modal onClose={()=>setModal(false)} title={`Pagamento — ${emp.find(e=>e.id===selEmp)?.nome}`}>
 <div style={{display:"grid",gap:12}}>
...
function Compras({CP,sv,svE,ES}){
 ...
 {confirmModal!==null&&<Modal onClose={()=>setConfirmModal(null)} title={`Confirmar Chegada — ${CP[confirmModal]?.item}`}>
