...
const VERSAO="2026.08.26 · build 30";
 ...
 return(
  <div>
   <div style={{marginBottom:14}}>
    <h2 style={{fontSize:20,fontWeight:800,color:"#f59e0b",margin:0}}>Materiais e Serviços Previstos</h2>
    <p style={{fontSize:12,color:"#94a3b8",margin:"4px 0 0",lineHeight:1.5}}>O que está previsto para cada ambiente da obra, com quantidade e unidade. {totalItens} item(ns).</p>
   </div>
   ...
   {etapas.map((e,i)=>{
    const qt=e.subs.reduce((s,x)=>s+x.itens.length,0);
    ...
    return(
     <div key={i} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,marginBottom:10,overflow:"hidden",boxShadow:"0 1px 3px #0001"}}>
      <div onClick={()=>setAberta(aberta===i?null:i)} style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,padding:"12px 14px",cursor:"pointer",background:"#f8fafc"}}>
       <span style={{fontSize:13,fontWeight:800,color:corEtapa(e.nome,filtrada)}}>{ab?"▾ ":"▸ "}{e.nome}</span>
       <span style={{fontSize:11,color:"#94a3b8"}}>{qt} item(ns)</span>
      </div>
      {ab&&e.subs.map((sb,j)=>(
       <div key={j}>
        <div style={{background:"#1e40af",color:"#fff",fontSize:11,fontWeight:700,padding:"6px 14px"}}>{sb.nome}</div>
        {sb.itens.map((c,k)=>(
         <div key={k} style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,padding:"9px 14px",borderBottom:"1px solid #f1f5f9"}}>
          ...
          <span style={{fontSize:13,fontWeight:800,color:"#f59e0b",whiteSpace:"nowrap"}}>
           {(Number(c.qtd)||0).toLocaleString("pt-BR")} <span style={{fontSize:11,color:"#64748b",fontWeight:600}}>{c.und||""}</span>
          </span>
         </div>
        ))}
       </div>
   ...
   {lista.length>0&&(
    <div style={{background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:10,padding:"11px 14px",fontSize:11,color:"#64748b",lineHeight:1.6,marginTop:14}}>
     Esta lista serve para conferência do que foi orçado por ambiente — quantidade e unidade de cada material ou serviço. Divergências entre o previsto aqui e o necessário em obra devem ser comunicadas pela aba Comunicação.
    </div>
   )}
  </div>
 );
